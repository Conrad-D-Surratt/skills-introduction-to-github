...
def load_books(file_name=BOOKS_FILE):
    ...
    # REPLACED: Removed the list comprehension
    books = []
...
def calculate_rental_details():
    ...
    # SPRINT 4 REQUIREMENT: Use datetime.now() for the log timestamp
...
def save_and_print_receipt(customer, book, cost, rental_date, due_date, current_time, file_name=RENTAL_HISTORY_FILE):
    ...
    # REPLACED: Removed the "*" parameter
...
def view_rental_history(file_name=RENTAL_HISTORY_FILE):
    ...
    print("\n--- Rental History ---")
    # REPLACED: Removed try/except to match beginner logic
    with open(file_name, "r") as file:
        for line in file:
            print(line.strip())
...
# --- RUN PROGRAM ---
# REPLACED: Removed if __name__ == "__main__"
main()